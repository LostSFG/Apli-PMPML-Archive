...
function displayTicket(passData) {
  ...
  startCountdown(passData.validityDate);
}

function startCountdown(validityDate) {
  const countdownEl = document.getElementById("countdown");
  const endTime = new Date(validityDate).getTime();
  ...
  window.countdownInterval = setInterval(() => {
    ...
    countdownEl.textContent = `Expires in ${hours}:${minutes}:${seconds}`;
  }, 1000);
}
...
function showPassHistory() {
  ...
  if (!passes.length) {
    passList.innerHTML = '<p>No pass history available.</p>';
  }
...
function toggleQR() {
  ...
  document.body.style.overflow = isOpen ? "hidden" : "hidden"; // keep locked
}